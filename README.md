...
ROAD (Robust Optimization with Array Data) is an open-source Python package for generic optimization modeling. ROAD models are constructed by variables, constraints, and expressions formatted as N-dimensional arrays, which are consistent with the NumPy library in syntax and operations, such as indexing and slicing, element-wise operations, broadcasting, and matrix calculation rules. It thus provides a convenient and highly readable way in developing optimization models and applications.
